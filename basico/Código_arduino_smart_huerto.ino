...
void loop() {
  if (configurado) {
    ...
    if (utc >= timeout) {
      enviarDatosH(humedad_aire, humedad_suelo, temperatura, luminosidad, lluvia);
    } else {
      enviarDatosActuales(humedad_aire, humedad_suelo, temperatura, luminosidad, lluvia);
    }
    delay(1000);
  } else {
    server.handleClient();
...
void enviarDatosH(String hum_aire, String hum_suel, String temp, String lum, String lluvia) {
...
void enviarDatosActuales(String hum_aire, String hum_suel, String temp, String lum, String lluvia) {
...
void iniciarSensores() {
  ...
  dht.begin();
  Serial.println("Sensor DHT correcto");
  pinMode(pinLluvia, INPUT);
  Serial.println("Sensor lluvia correcto");
  pinMode(pinLDR, INPUT);
  Serial.println("Sensor LDR correcto");
...
void recogerValores() {
  ...
  humedad_aire = String(dht.readHumidity());
  temperatura = String(dht.readTemperature());
  if (digitalRead(pinLDR)) {
    luminosidad = "Si";
  } else {
    luminosidad = "No";
  }
